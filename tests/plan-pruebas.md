...
|---|---|---|---|
| **TC-01** | Verificar inicio de sesión correcto con credenciales válidas de un solicitante. |  ✓ ||
| **TC-02** | Verificar que se rechace el inicio de sesión con contraseña incorrecta. |  ✓ ||
| **TC-03** | Verificar la creación correcta de una solicitud válida con equipos disponibles. | ✗ ||
| **TC-04** | Verificar que se rechace una solicitud de más de 3 equipos simultáneos. |  ✓ ||
| **TC-05** | Verificar que un usuario con 2 equipos activos no pueda solicitar otros 2, ya que superaría el máximo permitido. | ✗ ||
| **TC-06** | Verificar que un equipo en estado `MANTENIMIENTO` no pueda ser incluido en una nueva solicitud de préstamo. | ✓ ||
| **TC-07** | Verificar que un usuario con un préstamo atrasado no pueda crear una nueva solicitud. | ✓ ||
| **TC-08** | Verificar que una solicitud múltiple sea rechazada completamente si uno de los equipos está en mantenimiento o no disponible. | ✓ ||
| **TC-09** | Verificar que un encargado pueda registrar correctamente un nuevo usuario SOLICITANTE o ENCARGADO. | ✓ ||
| **TC-10** | Verificar que se rechace el registro de un usuario cuando faltan datos obligatorios. | ✓ ||
